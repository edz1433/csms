...
@section('title', 'IAR Report')
@section('header', 'Reports')
@section('subheader', 'Inspection and Acceptance Reports created from delivered supplies')

@section('content')
@include('reports.partials.tabs', ['active' => 'iar'])

<div x-data="iarReport({
        base: @js(route('reports.export', 'iar')),
        from: @js($filters['from']),
        to: @js($filters['to']),
        supplier_id: @js($filters['supplier_id']),
        acceptance_status: @js($filters['acceptance_status']),
        payment_status: @js($filters['payment_status']),
    ...
    <div class="relative z-20 bg-white rounded-xl border border-cpsu-border shadow-sm p-4 sm:p-5 mb-4" data-aos="fade-up">
        <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
            <div class="sm:col-span-4">
                <label class="block text-xs font-medium text-gray-500 mb-1">IAR Date Range</label>
                <div class="relative">
            ...
            <div class="sm:col-span-4">
                ...
                <select id="iar-supplier" x-model="supplier_id"
            ...
            <div class="sm:col-span-2">
                <label class="block text-xs font-medium text-gray-500 mb-1">Acceptance</label>
                <select x-model="acceptance_status"
                        class="w-full rounded-lg border border-cpsu-border px-3 py-2 text-sm bg-white focus:border-cpsu-green outline-none">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <div class="bg-white rounded-lg border border-cpsu-border p-4">
            <p class="text-xs text-gray-500 uppercase">IAR Records</p>
            <p class="text-2xl font-extrabold">{{ number_format($totals['total']) }}</p>
        </div>
        <div class="bg-white rounded-lg border border-cpsu-border p-4">
            <p class="text-xs text-gray-500 uppercase">Paid</p>
            <p class="text-2xl font-extrabold text-cpsu-green">{{ number_format($totals['paid']) }}</p>
        </div>
        <div class="bg-white rounded-lg border border-cpsu-border p-4">
            <p class="text-xs text-gray-500 uppercase">Unpaid</p>
            <p class="text-2xl font-extrabold text-amber-600">{{ number_format($totals['unpaid']) }}</p>
        </div>
        <div class="bg-white rounded-lg border border-cpsu-border p-4">
            <p class="text-xs text-gray-500 uppercase">Partial</p>
            <p class="text-2xl font-extrabold text-gray-800">{{ number_format($totals['partial']) }}</p>
        </div>
    </div>

    @include('reports.partials.pdf-frame', [
        'title' => 'IAR Report',
        'emptyText' => 'Set the filters above, then click Generate PDF to preview the IAR report.',
    ])
</div>

@push('scripts')
<script>
  function iarReport(cfg) {
    return {
      from: cfg.from, to: cfg.to, supplier_id: cfg.supplier_id || '',
      acceptance_status: cfg.acceptance_status || '', payment_status: cfg.payment_status || '',
      fund_cluster_id: cfg.fund_cluster_id || '', account_title_id: cfg.account_title_id || '', url: '',
      init() {
        var self = this;
        new TomSelect('#iar-supplier', { create: false, allowEmptyOption: true });
        new TomSelect('#iar-fund', { create: false, allowEmptyOption: true });
        new TomSelect('#iar-account', { create: false, allowEmptyOption: true });
      ...
      generate() {
        ...
        if (this.acceptance_status) { params.acceptance_status = this.acceptance_status; }
        if (this.payment_status) { params.payment_status = this.payment_status; }
